"""Modelo CP-SAT de OR-Tools para el problema maestro de cajas candidatas."""
...
def solve_for_thickness(
    ...
    *,
    ...
    """Optimiza un grosor global fijo sobre un universo finito de candidatos.

    La asignación provista es una incumbente protegida, no sólo una indicación.
    Si no se provee, las dimensiones internas actuales forman un respaldo garantizado.
    Una corrida con límite de tiempo nunca puede devolver una solución peor.
    Con ``target_total_mills``, CP-SAT resuelve factibilidad bajo ese costo; si
    no alcanza el objetivo, se devuelve la incumbente protegida.

    ``free_product_codes`` define opcionalmente una búsqueda de vecindario
    grande: sólo esos SKU pueden cambiar y los demás quedan fijos a su
    geometría incumbente. Requiere ``initial_assignment``. Los tiers de costo
    se calculan sobre todos los SKU, incluida la parte fija.

    ``allowed_internals_by_product`` puede restringir aún más SKU seleccionados
    a un conjunto pequeño de geometrías, por ejemplo movimientos binarios entre
    incumbente y objetivo. Requiere una incumbente. La geometría incumbente se
    agrega a cada conjunto provisto para que el respaldo protegido sea factible.
    Los productos fuera del mapeo permanecen sin restricción; los fijados por
    ``free_product_codes`` siguen fijos aun si el mapeo contiene geometrías adicionales.

    Un ejecutor LNS repetido puede aportar un universo exacto completo ya
    calculado para no enumerar la misma grilla antes de cada subproblema. Ese
    universo debe retener toda geometría incumbente o permitida explícitamente.
    """
    ...
    for code, internals in supplied_allowed_internals.items():
        ...
        # Una colección vacía significa deliberadamente "no moverse". Agregar
    # la incumbente garantiza el respaldo protegido en todo LNS restringido
        # sin obligar al llamador a repetirla en cada conjunto de movimientos.
    ...
    # Los diseños retenidos sobreviven la deduplicación por firma y la poda por
    # dominancia. Aun así se valida compatibilidad por SKU: una geometría puede
    # ser factible para un producto e inválida para el SKU que se le asigna.
    ...
    # La restricción LNS se aplica después de calcular mínimos globales de
    # pallets. De otro modo, fijar un SKU incumbente elevaría artificialmente
    # ``minimum_possible_pallets`` y relajaría el presupuesto solicitado.
    ...
    # Reconstruye la incumbente con candidatos canónicos del universo. Los
    # diseños retenidos aseguran que sobrevivan deduplicación y dominancia.
    ...
    # Con grosor fijo, toda unidad tiene el mismo precio de tier 1, sin importar
    # diseño o planta. Esa parte es constante; sólo los descuentos por umbral
    # sobre todas las unidades dependen de la asignación.
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        for plant in PLANTS:
            ...
            # Bajo el primer umbral esta caja/planta nunca logra descuento: la
            # constante global ya cubre su costo base y no requiere variables.
    ...
    # Procurement es una tabla de descuento sobre todas las unidades. Se expresa como
            # costo de tier 1 menos un descuento acumulado por cada umbral
    # alcanzado. Equivale a la formulación anterior de selección exclusiva, con menos
            # variables y sin partición mutuamente excluyente de volumen.
    ...
    # La incumbente queda deliberadamente fuera del objetivo. CP-SAT la usa
    # como punto de reparación en vez de descartar una indicación informativa.
...
def solve_all_thicknesses(
    ...
    **solver_kwargs: object,
    ...
    """Resuelve todos los grosores globales permitidos y devuelve el menor costo."""
