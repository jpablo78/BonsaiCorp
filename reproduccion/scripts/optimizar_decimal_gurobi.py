...
from gurobi_proto import bound_usd, gurobi_from_proto, optional_model_attr, status_name
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("salida_decimal")
    )
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    model, variables = gurobi_from_proto(proto)
    ...
    model.optimize()
    if model.SolCount < 1:
        raise RuntimeError(f"Gurobi no encontró solución decimal; estado={model.Status}")
    ...
    if expected_mills != costs.total_mills:
        raise AssertionError(
            f"el objetivo Gurobi {expected_mills} difiere del costo independiente {costs.total_mills}"
        )
    ...
    if checked_costs.total_mills != costs.total_mills:
        raise AssertionError("la escritura y lectura del CSV decimal cambió el costo total")
    if set(checked_assignment) != set(assignment):
        raise AssertionError("la escritura y lectura del CSV decimal cambió los productos asignados")

    baseline_usd = 209_235_093.6821712
    payload = {
        "modo": "optimizacion_decimal",
        ...
        "runtime_seconds": optional_model_attr(model, "Runtime"),
        "nodes": optional_model_attr(model, "NodeCount"),
        "relative_gap": optional_model_attr(model, "MIPGap"),
        "best_bound_usd": bound_usd(model, builder.objective_scale_mills),
