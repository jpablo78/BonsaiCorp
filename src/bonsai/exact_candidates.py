"""Exact integer-mm candidate enumeration for the documented FAQ #10 model.

The feasible global dimension grid is small enough to enumerate.  Bit masks
make it possible to calculate the complete set of compatible SKUs for every
box without testing every product in the inner loop.  Designs with the same
capacity and compatibility set are objective-equivalent; dominated designs
are then removed safely.
"""
...
# Exact rational forms of the percentages documented for each thickness.
...
def exact_axis_bounds(original_mm: float, thickness_mm: float) -> tuple[int, int]:
    """Inclusive integer output bounds for a possibly fractional source axis."""
...
def _prefix_masks(values_by_product: tuple[float, ...]) -> tuple[tuple[float, ...], tuple[int, ...]]:
    """Build cumulative SKU masks for queries of the form value <= threshold."""
...
def _remove_dominated(signatures: list[_Signature]) -> list[_Signature]:
    """Remove boxes dominated by a higher-capacity compatibility superset."""
...
def generate_exact_candidates(
    ...
    *,
    ...
    """Enumerate the complete integer grid and return its objective-relevant designs."""
    ...
    for ordinal, signature in enumerate(signatures):
        ...
        # This is deliberately redundant: it makes any mismatch between the
        # accelerated bit-mask logic and the independent geometry rules fail
        # before a solver or submission can use the candidate.
        fit_oracle = (
            flexible_product_fits_candidate if flexible_layout else product_fits_candidate
        )
        if any(
