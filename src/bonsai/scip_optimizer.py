"""Alternative exact MIP master model using OR-Tools MPSolver/SCIP.

The candidate universe and the independent cost evaluator are shared with the
CP-SAT implementation.  Procurement uses cumulative all-units discount
thresholds: a binary indicates whether a threshold is reached and an integer
auxiliary is the complete candidate/plant volume when it is reached (zero
otherwise).  This keeps the model linear and exactly matches ``costs.py``.
"""
...
def solve_with_scip(
    ...
    *,
    ...
    """Solve the complete exact integer-mm master problem with SCIP.

    A supplied warm start is also a protected incumbent: the MIP is bounded by
    its independently evaluated cost and the method never returns a worse
    assignment.  ``max_extra_pallets`` is measured from the global per-SKU
    minimum over the complete exact universe, as in the CP-SAT model.

    ``free_product_codes`` and ``allowed_internals_by_product`` define a
    restricted large-neighbourhood problem with the same semantics as
    :func:`bonsai.optimizer.solve_for_thickness`.  Products outside the free
    set are removed from the MIP completely: their incumbent freight is an
    objective constant and their incumbent demand is folded into the fixed
    volume of each candidate/plant procurement tier.  The incumbent geometry
    is always added to every explicitly allowed set.

    ``max_changed_products`` and ``min_changed_products`` are search controls,
    not commercial constraints. They bound the Hamming distance from
    ``initial_assignment`` in product-to-physical-box assignments, for exact
    local branching around a validated incumbent.
    """
    ...
    # Apply LNS restrictions only after computing the global per-SKU pallet
    # minima.  This keeps ``max_extra_pallets`` identical to the full master
    # model.  Non-free products become singleton rows here and are subsequently
    # absorbed as constants instead of receiving assignment variables.
    for product_index, product in enumerate(data.products):
    ...
    # Safe reduced-cost style filtering.  If p is assigned to c, every unit at
    # a plant pays at least the price attainable when *all* currently compatible
    # products use c there.  Adding that packaging lower bound to the exact
    # freight cost gives a separable lower bound for every assignment arc.  An
    # arc whose contribution plus the best lower bounds for all other products
    # exceeds the protected incumbent can never occur in an improving solution.
    # Repeating after deletions tightens candidate maximum volumes and therefore
    # the lower bound, without changing the feasible improving set.
    ...
    if not relax_integrality:
        parameter_lines = [
            ...
            # SCIP stores LP rows as doubles.  Objective coefficients are scaled
            # below, but a tight feasibility tolerance remains useful at a hard
            # target boundary.
    ...
    # A local-branching neighbourhood is expressed only in rows that can vary.
    # Singleton rows retain their incumbent geometry (the incumbent arc is
    # explicitly preserved above), so their Hamming-distance contribution is
    # always zero and can be folded out.
    ...
    if min_changed_products is not None and min_changed_products > len(
        ...
        # Keep the requested neighborhood exact: this impossible row lets SCIP
        # report infeasibility rather than silently widening it.
    ...
    # Constant-fold procurement for candidate/plant pairs whose volume cannot
    # change in this restricted model.  Previously these fixed-only types still
    # received a volume variable and up to eight threshold auxiliaries.  A
    # short LNS with only a few genuinely free arcs could therefore carry
    # hundreds of useless integer variables.  ``base_packaging_mills`` already
    # prices all demand at tier 1, so only the exact all-units discount delta is
    # added here.
    ...
    for candidate_index, candidate_assignments in enumerate(variables_by_candidate):
        ...
        for plant in PLANTS:
            ...
            if not positive_assignments:
                # Its exact procurement delta was folded into the objective
                # constant above; there is no decision at this plant/type.
                continue
            # With no fixed demand and only one positive-volume decision, the
            # plant/type procurement cost has exactly two states (zero or the
            # SKU's complete volume).  Put that exact delta directly on x and
            # avoid one volume variable plus up to eight tier auxiliaries.
            ...
            for tier_index, tier in enumerate(DISCOUNT_TIERS[1:], start=1):
                ...
                # reached=1 iff volume >= threshold.
                ...
                # discounted_volume = volume when reached, otherwise zero.
    ...
    if export_model_path is not None:
        ...
        # OR-Tools' text LP and MPS writers print large coefficients with a
        # limited number of significant digits.  They are suitable for manual
        # inspection, but a cross-solver benchmark needs a lossless format.
        # ``.pb`` therefore serializes the native MPModelProto, whose doubles
        # represent the integer coefficients exactly at this magnitude.
    ...
    # Before SCIP has processed the root relaxation MPSolver can expose its
    # internal -infinity sentinel as the bound.  Costs are non-negative, so a
    # negative/non-finite value is not a useful certified bound to report.
    best_bound = (
        raw_best_bound
