"""Exact enumeration on a configurable decimal internal-dimension grid.

The decimal relaxation preserves the strict per-axis fit/headspace, ECT,
fixed pallet orientation and product-volume rules.  Axis values are generated
from state-change events instead of scanning every grid point.  This makes a
micro-millimetre grid practical and, importantly, includes the exact discrete
neighbours of every fit, headspace and pallet-capacity breakpoint.
"""
...
def _compressed_axis_values(
    ...
    """Keep the largest grid value in every axis-state cell.

    Within such a cell a larger dimension preserves axis compatibility and
    pallet capacity while weakly improving volume and, for horizontal axes,
    ECT.  It therefore dominates every smaller grid value in the same cell.

    A state can change only at a product lower bound, immediately after a
    product upper bound, or immediately after a pallet-count threshold.  It is
    therefore sufficient to evaluate those event points and their grid
    neighbours; scanning the potentially enormous full axis grid is not
    necessary.
    """
    ...
    for count in range(1, maximum_count + 1):
        # count * external_units <= pallet_units is exactly the condition for
        # at least ``count`` boxes along this pallet axis.
...
def generate_tenth_mm_candidates(
    ...
    *,
    ...
    """Backward-compatible convenience wrapper for the 0.1-mm grid."""
