"""Generate a compact, auditable universe of feasible integer-mm box designs."""
...
def _integer_fit_bounds(value: float) -> tuple[int, int]:
    """Return the inclusive integer bounds for the FAQ's ±10% adjustment."""
...
def _pallet_edge_values(
    ...
    """Internal sizes at which one more box may fit along a pallet axis."""
...
def _pallet_aligned_variants(
    ...
    *,
    ...
    """Generate volume-preserving integer designs near pallet-capacity steps.

    Two internal dimensions are placed at a feasible pallet edge.  The third
    is the smallest integer dimension that preserves the product volume.  The
    construction is repeated for each choice of dependent dimension so that
    headspace can occur on any axis.
    """
...
def _group_compromise_variants(
    ...
    *,
    ...
    """Find pallet-aligned boxes feasible for every product in one SKU group.

    The current incumbent defines groups of SKUs that already share a design.
    This routine searches the intersection of their ±10% dimension bands and
    uses the largest product volume when constructing a box.  Retained designs
    are checked against every SKU, including headspace and compression.
    """
...
def generate_candidates(
    ...
    *,
    ...
    """Build a deterministic candidate universe.

    Every current internal profile is retained, guaranteeing a feasible
    no-consolidation solution.  Additional boxes are component-wise maxima of
    high-volume profile pairs and volume-preserving, pallet-aligned variants
    of high-volume profiles.  Optional seed profiles and SKU groups can focus
    those variants around designs used by an incumbent.  The latter capture
    discrete capacity jumps such as fitting an extra box along a pallet
    dimension.  The CP-SAT solution is exact over this explicit, reproducible
    candidate universe.
    """
    ...
    # Retain every warm-start design itself as well as its neighborhood. This
    # guarantees that a valid incumbent is available to CP-SAT as a hint.
    all_designs = (
        anchor_designs
        | extra_designs
